...
## 13. Track A1 — Media list navigation-scoped

Bug thật, quan sát hằng ngày: reload một trang, danh sách media của Media Sniffer không xoá mà cứ dồn thêm, trong khi link cũ (thường là signed URL) đã hết hạn. Gốc: `DetectedMedia` là state *navigation-scoped* nhưng lại sống trong `chrome.storage.local` *vĩnh viễn*, lọc lại lúc đọc theo `tabUrl` — và `chrome.tabs.onUpdated`'s `changeInfo.url` (tín hiệu duy nhất từng dùng để refresh) không bao giờ fire khi reload CÙNG URL, nên không có tín hiệu nào để dọn.

- **Permission mới: `webNavigation`** ([manifest.config.ts](../manifest.config.ts)) — `chrome.webNavigation.onCommitted` là event duy nhất fire trên mọi navigation commit kể cả reload cùng URL.
- **`DetectedMedia.tabId?: number`** ([features/media/store.ts](../src/adapters/browser-extension/features/media/store.ts)) — tag mới, tách biệt với `tabUrl`/`pageUrl` (2 field đó vẫn phục vụ hiển thị/scoping-theo-URL như cũ). Gán từ `chrome.webRequest`'s `tabId` (bỏ qua `-1`, sentinel "không có tab" cho request page tự proxy qua Service Worker) hoặc `chrome.runtime.MessageSender.tab.id` ở 2 nguồn detection còn lại (`report-dom-media`/`report-main-world-media`).
- **`clearDetectedMediaForTab(tabId, cache)`** ([features/media/store.ts](../src/adapters/browser-extension/features/media/store.ts)) — xoá mọi entry mang đúng `tabId`; entry không có `tabId` (không gán được tab thật) bị bỏ qua, cùng tinh thần "best-effort, không đoán" `tabUrl` đã có. Test bằng fake `CacheService` ([store.test.ts](../src/adapters/browser-extension/features/media/store.test.ts)).
- **[features/media/state-lifetime.background.ts](../src/adapters/browser-extension/features/media/state-lifetime.background.ts)** (file mới, đổi tên thành `state-lifetime.background.ts` ở A2 khi nó gánh thêm việc chung cho cả `storage.session`/`storage.tab`) — lắng `chrome.webNavigation.onCommitted`, lọc `frameId === 0` (chỉ top-level của tab, không phải iframe con navigate), gọi `clearDetectedMediaForTab`. Không bọc Module — không có toggle bật/tắt nào hợp lý cho hành vi này, luôn cài đặt ở startup như `webrequest-media-observer.background.ts`.
- **✅ Đã xác nhận bằng Chrome thật (2026-08-08), đủ cả 4 case**: reload cùng URL → list chỉ còn media của lượt load mới; 2 tab độc lập, reload tab A không ảnh hưởng tab B; iframe con tự điều hướng không xoá nhầm list của trang cha; media trong iframe quảng cáo cross-origin vẫn bị xoá đúng khi tab đó reload.
- **"Side Panel state theo tab" (nghi vấn A1 ban đầu) — xác nhận KHÔNG có bug, không cần vá.** ROADMAP từng ghi nghi vấn `[§6.4]` (Side Panel "reset state mỗi lần chuyển tab") nhưng không tìm được bug cụ thể nào ghi lại, và đọc code `ui/side-panel/main.ts` cũng không thấy state nào thực sự mất — `items`/`activeDownloads`/`selectedVariantUrl` đều tự derive đúng theo active tab qua `chrome.tabs.onActivated`. User tự test trên Chrome thật (2026-08-08) đúng theo "Xong khi" đã đặt ra: chuyển tab đi rồi quay lại → Side Panel giữ đúng state của tab đó; download job sống qua cả reload lẫn chuyển tab — **confirmed, hành vi đã đúng sẵn từ trước**.

## 14. Track A2 — `synapseApi.storage.session`/`.tab`

Platform có 3 lifetime (permanent/tab/navigation) mà script chỉ có 1 (`storage.rw`) thì script thành công dân hạng hai. Đóng khoảng cách mà **không breaking change**: `storage.get/set/remove/keys` giữ nguyên hành vi permanent hiện có (không đổi tên thành `.local` dù ROADMAP từng nhắc tới cái tên đó — không cần thiết và tránh phá vỡ script đã viết), chỉ thêm 2 namespace con cùng scope `storage.rw` (chiều *vòng đời*, không phải chiều *quyền*, nên không sinh scope mới).

- **Phát hiện quan trọng nhất, tiết kiệm cả một round-trip thiết kế**: `rpc-handler.ts`'s `resolveMethodHandler` **đã hỗ trợ sẵn method tên có dấu chấm** (tiền lệ `net.mock.add`) — nên `storage.session.get`/`storage.tab.get` chỉ cần thêm entry `API_METHODS` với `method: 'session.get'`/`'tab.get'`, không cần sửa cơ chế dispatch.
- **[kernel/synapse-api.ts](../src/kernel/synapse-api.ts)** — `SynapseKeyValueApi` (get/set/remove/keys) tách ra làm shape dùng chung; `SynapseStorageApi extends SynapseKeyValueApi` thêm `tab`/`session: SynapseKeyValueApi`.
- **[module-registry/script-storage.ts](../src/adapters/browser-extension/module-registry/script-storage.ts)** — 2 prefix mới `script-tab:`/`script-session:`, **cố ý KHÔNG lồng dưới `script:<moduleId>:`** (prefix anh em không bao giờ collide với prefix gốc, kể cả khi user key trùng tình cờ với `tab:`/`session:`). `tabId` nằm ở segment riêng, tách bằng `:`, nên sweep không nhầm tab 1 với tab 10. `createScriptStorage(moduleId, tabId, backend)` đổi signature (thêm `tabId` — hàm nội bộ, không phải public API, nên tự do đổi); không có `tabId` (background Module) → trả stub reject với lỗi rõ ràng, cùng khuôn `synapse-api-host.ts`'s `backgroundPageStub`. `clearScriptStorage` (xoá 1 script khi bị gỡ) mở rộng quét cả 2 prefix mới, mọi tabId. Test-first, 16 case ([script-storage.test.ts](../src/adapters/browser-extension/module-registry/script-storage.test.ts)).
- **`clearTabScopedStorageForTab`/`clearSessionScopedStorageForTab`** (cùng file) — sweep CROSS-MODULE theo tabId, gọi từ `state-lifetime.background.ts`: tab đóng (`chrome.tabs.onRemoved`) → xoá cả `tab` lẫn `session`; navigation commit (kể cả reload cùng URL) → chỉ xoá `session`, `tab` phải sống sót qua navigation (đó là điểm khác biệt duy nhất giữa 2 lifetime).
- **3 transport đều cập nhật** (checklist `userscript-api` skill's "Adding a method"): `synapse-api-host.ts` (truyền `context.tabId`), `user-script-shim.ts` (cả nhánh thật lẫn nhánh `__synapseWrongHandle` chống mạo danh global), `content-scripts/rpc-client.ts` (dom Module), `kernel/service-injector.ts`'s `unavailableSynapseApi` stub.
- `docs/types/synapse-userscript.d.ts` regenerate; ví dụ mới trong [user-scripts.md](user-scripts.md) + [docs/examples/test-storage-lifetime.js](examples/test-storage-lifetime.js).
- **✅ Đã xác nhận bằng Chrome thật (2026-08-08), cả 3 transport**: `storage.tab` qua chuyển/đóng tab, `storage.session` reset khi reload, reject rõ ràng từ background Module, hoạt động từ uploaded script VÀ từ dom Module (content-script RPC transport), xoá script dọn sạch cả 3 namespace. Test dom Module cần wire tạm `trustedScopes['reader-mode-converter']` trong `background/index.ts` (đã revert ngay sau khi xác nhận) — lộ ra Open Point thật: **không một dom Module bundled nào từng gọi được method có scope**, vì `background/index.ts`'s `trustedScopes` chỉ build từ `BACKGROUND_MODULES`, cố ý bỏ qua `BUNDLED_MODULES` (import nó thật sẽ kéo `@mozilla/readability`+Turndown vào service worker bundle cho một map hiện đang luôn rỗng — không riêng Track A2, sẽ chặn NGANG mọi scope khác nếu một dom Module tương lai cần). Chưa vá — ghi vào ROADMAP's Open Points, xử lý khi Track B có dom Module thật cần scope.

## 15. Track A3 — TTL / GC cho state permanent

Gộp cùng Track A vì cùng là bài toán vòng đời (trước đây là Open Point riêng: checkpoint mồ côi sống mãi, blob Review session không bao giờ dọn).

- **[shared/ttl.ts](../src/shared/ttl.ts)** — `isExpired(createdAt, maxAgeMs, now)`, hàm thuần duy nhất trong toàn bộ tính năng này (phần còn lại đụng `indexedDB`/`chrome.alarms`, không dựng harness được — đúng carve-out CLAUDE.md §4.2). Test-first ([ttl.test.ts](../src/shared/ttl.test.ts)).
- **[utils/blob-store.ts](../src/adapters/browser-extension/utils/blob-store.ts)** — `StoredBlob` thêm `createdAt`, **tự stamp bên trong `putBlob`** (không đổi call site nào ở `item-form-view.ts`/`review-handoff.ts` — cả 2 predate field này). `sweepStaleReviewBlobs(maxAgeMs)` quét cursor, chỉ xoá key bắt đầu bằng `review:` — **cố ý KHÔNG đụng blob của `http-mock`** (id là uuid trần, dùng chung 1 IndexedDB object store nhưng vòng đời gắn với `MockConfig` rule, không phải theo tuổi). Blob cũ chưa từng có `createdAt` (ghi trước khi field này tồn tại) coi như đã hết hạn (`?? 0`) thay vì sống mãi vì "không biết tuổi".
- **[features/media/download/checkpoints.ts](../src/adapters/browser-extension/features/media/download/checkpoints.ts)** — `orphanedCheckpointIds(checkpoints, detectedMediaIds)` (hàm thuần, test-first, [checkpoints.test.ts](../src/adapters/browser-extension/features/media/download/checkpoints.test.ts)): checkpoint mồ côi = `jobId` không khớp `DetectedMedia.id` nào còn sống (đúng quan hệ `ui/side-panel/main.ts` đã dùng để tìm checkpoint của 1 entry). Không có tiêu chí tuổi riêng — mồ côi là mồ côi ngay khi phát hiện, không đợi 7 ngày. `sweepOrphanedCheckpoints(detectedMediaIds)` nhận Set có sẵn từ caller thay vì tự import `store.ts` — giữ nguyên file này không có cross-feature import mới.
- **[background/storage-gc.ts](../src/adapters/browser-extension/background/storage-gc.ts)** (file mới) — `chrome.alarms`, không phải `setTimeout` (service worker không sống đủ lâu). Sweep 1 lần/ngày (`delayInMinutes: 1` lần đầu, `periodInMinutes: 1440` sau đó) — tần suất dư thừa so với TTL 7 ngày, chọn vì đơn giản chứ không phải vì cần chính xác. TTL hardcode 7 ngày (`REVIEW_BLOB_TTL_MS`) — phần UI chỉnh TTL chặn bởi chưa có trang Settings toàn cục (D5).
- **Bugfix ngay trong lượt test đầu (trước khi user kịp xác nhận gì): sweep không có cách nào quan sát được.** `runStorageGcSweep()` ban đầu chỉ log khi LỖI — một sweep chạy thành công nhưng không xoá gì (đúng thiết kế khi không có gì mồ côi/hết hạn) và một sweep CHƯA TỪNG chạy nhìn giống hệt nhau ở console, nên user không kết luận được gì khi test thủ công lần đầu ("chỉnh GC về 1 phút, không thấy gì bị xoá"). Vá 2 phần: (a) `sweepOrphanedCheckpoints`/`sweepStaleReviewBlobs` đổi từ `Promise<void>` sang trả về cái gì đã xoá (`string[]`/`number`); (b) `runStorageGcSweep()` luôn `console.log` kết quả, và thêm `globalThis.__synapseStorageGc.runSweep()` (cùng khuôn debug handle `__synapseSniffer`) để gọi tay ngay lập tức từ console thay vì đợi `chrome.alarms` (tới 1 ngày) hay phải sửa hardcode `GC_SWEEP_INTERVAL_MINUTES` trong source. Cách hướng dẫn ban đầu ("sửa `createdAt` qua DevTools' IndexedDB viewer") cũng sai — panel đó không cho sửa inline; TEST_PLAN đổi sang đoạn script `indexedDB` chạy thẳng ở console.
- **Permission mới: `alarms`** ([manifest.config.ts](../manifest.config.ts)).
- **✅ Đã xác nhận bằng Chrome thật (2026-08-08)**: checkpoint mồ côi bị dọn qua `__synapseStorageGc.runSweep()`; permission `alarms` hoạt động; blob Review hết hạn (>7 ngày) bị xoá VÀ blob `http-mock` (rule dùng file tự upload thật, không phải `MOCK_FILES` bundled) sống sót dù cũng bị lùi tuổi — xác nhận filter theo prefix `review:` hoạt động đúng như thiết kế.

## 16. Track B2e — Catalog scope: bỏ trần đếm, thay bằng bất biến tài nguyên

`ALL_SCOPES.length <= 10` (`scopes.test.ts`) đo sai chiều: số dòng user thấy phụ thuộc script khai bao nhiêu scope, không phụ thuộc catalog có bao nhiêu, và ở biên nó ép gộp những năng lực khác bản chất vào một scope — ép consent UI nói dối, đúng thứ nó sinh ra để ngăn.

- **`ScopeDefinition.unboundedReason?: string`** ([kernel/scopes.ts](../src/kernel/scopes.ts)) — bắt buộc khi `requiresMatch` là `false`: giải thích vì sao scope đó không có chiều tài nguyên để bind `match`. Điền cho cả 7 scope không `requiresMatch` (`storage.rw`, `page.dom`, `ui.render`, `page.fetch`, `files.save`, `media`, `secrets.use`).
- **Bất biến máy kiểm mới** ([scopes.test.ts](../src/kernel/scopes.test.ts)): mọi scope phải có `requiresMatch: true` HOẶC `unboundedReason` không rỗng — thay hẳn cho trần đếm cũ.
- TDD tầng thuần: test viết trước (đỏ vì field chưa tồn tại), code tới xanh. `npm test`/`typecheck`/`build` sạch, không đụng runtime nào khác.

## 17. Track B2a/B2b — `net.mock` nhận `action` (block/rewrite-request), `net.mock.debugger` grant riêng

Trước đây `net.mock.add` hard-code `action: 'fake-response'` + `mechanism: 'main-world'` — script chỉ với tới 1 trong các tổ hợp hành vi mà builtin "HTTP Mock & Rewrite" panel đã hỗ trợ sẵn cho người dùng thao tác tay.

- **`chooseMechanismForScriptRule(action, { rewriteBody?, matchAnyResourceType? })`** ([shared/http-mock.ts](../src/shared/http-mock.ts), hàm thuần, test-first) — script chỉ khai **ý định** (`action` + 2 hint), platform luôn tự chọn mechanism: `block` → luôn `dnr` (đã catch mọi resource type, ngang `debugger`, không tốn banner); `rewrite-request` → `main-world` khi có `rewriteBody` mà không cần `matchAnyResourceType`, `dnr` khi cần `matchAnyResourceType` mà không rewrite body, `debugger` CHỈ khi cả hai cùng lúc; `fake-response` giữ nguyên `main-world`.
- **Scope mới `net.mock.debugger`** ([kernel/scopes.ts](../src/kernel/scopes.ts), `enforced`, `requiresMatch: true`) — bắt buộc THÊM vào `net.mock` cho đúng 1 combo trên (`rewrite-request` + `rewriteBody` + `matchAnyResourceType`). Case thật, không lý thuyết: mock một `<script src>` bundle hoặc một `<img>` ảnh lớn nhúng thẳng HTML tag — cả hai không phải request do `fetch`/XHR khởi tạo nên `main-world` không vá tới, và `dnr` không rewrite được body của bất kỳ resource nào. Consent line nói thẳng về banner "đang debug tab" permanent Chrome tự hiện.
- **`rpc-handler.ts`** — thêm 1 khối kiểm tra điều kiện theo namespace/method (cùng khuôn khối `secrets.use` đã có sẵn cho `net.request`/`ai.ask`): với `net.mock.add`, chạy ĐÚNG `chooseMechanismForScriptRule` trên `req.args` TRƯỚC khi dispatch — mechanism resolve ra `'debugger'` mà chưa có `net.mock.debugger` (cùng `resourceUrl` với `net.mock`) thì fail-closed. `net-mock-host.ts` tin enforcement đã xảy ra ở đây, không tự kiểm lại (§3.4 — một điểm enforce duy nhất).
- **`kernel/synapse-api.ts`** — `SynapseMockRuleOptions` thêm `action?`, `rewriteUrl?`, `rewriteMethod?`, `rewriteHeaders?: Record<string,string>`, `rewriteBody?`, `matchAnyResourceType?`; `SynapseMockRule` (trả về từ `list()`) thêm `action` bắt buộc + các field rewrite tương ứng, `fakeStatus` đổi thành optional (chỉ có nghĩa với `fake-response`).
- **`module-registry/net-mock-host.ts`** — `performMockAdd` build `MockConfig` theo `action` thay vì hard-code; `toSynapseMockRule` chiếu ngược đúng field theo action, dùng `parseHeadersJson` (export mới từ `shared/http-mock.ts`) để trả `rewriteHeaders` về dạng object qua wire thay vì JSON-text lưu trong storage.
- Regenerate `docs/types/synapse-userscript.d.ts` + `docs/synapse-ai-context.md` (snapshot test tự bắt lệch). Mở rộng [`docs/examples/test-net-mock.js`](examples/test-net-mock.js) với cả 3 scenario (fake-response/block/rewrite-debugger). Thêm mục vào [TEST_PLAN.md](TEST_PLAN.md) — CHƯA verify bằng Chrome thật (banner thật, block thật ở network layer, rewrite thật trên `<script>`/`<img>` tag).
- `.domain.md` của `features/http-mock/` cập nhật chính sách chọn mechanism + trạng thái parity mới.
