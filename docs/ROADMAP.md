...
## 11. Pivot: Userscript Platform — Phase 0 + 1 đã implement, Phase 2→7 còn là KẾ HOẠCH
...
`0 → 1 → 2 → 3 → 4 → 5 → 6 → 7`. **Phase 0 và 1 xong. Kế tiếp: Phase 2 (keystone).**

Phase 0 và 1 **độc lập hoàn toàn với phần pivot** — không cần chốt thêm gì. Phase 2 là keystone, mọi phase sau phụ thuộc nó. Mỗi phase có "Xong khi" riêng nên dừng giữa chừng ở ranh giới phase là an toàn.

**Điểm "có thể phải đảo 1↔2" đã hết hiệu lực** (Phase 1 xong rồi), nhưng bản thân câu hỏi vẫn còn và giờ là việc ĐẦU TIÊN của Phase 2: Open Point "[§11 Phase 2] Upload 2 user script cùng lúc" (khu vực "Chưa verify bằng browser thật") — nếu `const __SYNAPSE_MODULE_ID__` ở top-level shim thật sự trùng khai báo và script thứ 2 `SyntaxError`, thì hôm nay platform chỉ chạy được ĐÚNG 1 user script. Thử mất 2 phút (upload 2 file `.js` bất kỳ, mở console USER_SCRIPT world).
...
- **[§11] Pivot Userscript Platform — Phase 0 + 1 xong, còn Phase 2→7** (đã chốt hướng, mỗi phase có "Xong khi" riêng). Ba lỗi đã xác định qua đọc code, cố ý KHÔNG vá lẻ mà để Phase 2 dọn một lượt: `synapse.bus.on()` gãy câm lặng (handler là function, không qua structured clone — `rpc-client.ts` đã biết và cố tình không proxy `bus`, nhưng shim vẫn expose); `needs: ['net'|'dom']` là no-op (có trong `Capability`, không có service trong `ModuleContext.services`); `ai.*` throw vì `background/index.ts` chưa wire factory.
...
- **[§11 Phase 2]** Upload 2 user script cùng lúc: nghi `const __SYNAPSE_MODULE_ID__` ở top-level shim bị trùng khai báo (mọi script chia sẻ một USER_SCRIPT world — chính doc comment của shim ghi "not per-script sub-isolation within that world") → script thứ 2 `SyntaxError`. Nếu đúng thì **hiện chỉ dùng được 1 user script**. Cách thử: upload 2 file `.js` bất kỳ, mở console của USER_SCRIPT world. Là việc ĐẦU TIÊN của Phase 2 (§11.9) — Phase 1 đã xong nên không còn chuyện đảo thứ tự nữa, nhưng câu trả lời quyết định `buildShimSource` phải bọc IIFE ngay từ đầu Phase 2 hay không.
