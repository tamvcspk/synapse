...
### 8.12. State management cho download job — persist + resume sau khi bị ngắt — Đã implement (chỉ HLS)
...
- **§8.12 — vòng test thật đầu tiên đã bắt được 1 bug thật, đã vá (`OpfsRun.commit()`, xem [LESSONS.md](LESSONS.md)), CHƯA re-test sau vá:** resume ban đầu fail 100% vì checkpoint chưa từng làm bytes durable trước khi lưu — sửa bằng `commit()` (đóng+mở lại writable trước mỗi checkpoint). Cần verify lại từ đầu: resume thật sự tiếp tục đúng từ giữa file (không hỏng byte ở đúng ranh giới đã `truncate()`), interval 20s có đủ giãn để `commit()`'s chi phí O(kích thước file) không làm chậm thấy rõ trên stream lớn (nhiều trăm MB+), `sweepStaleOpfsRuns`'s `keepRunIds` không vô tình xoá nhầm file đang cần giữ, và checkpoint's message-ordering fix (`pendingCheckpointSave` awaited trước `clearCheckpoint`) thật sự chặn đúng race. Rủi ro RIÊNG vẫn còn treo, chưa chạm tới được ở vòng test đầu (vì lỗi trên chặn trước): `createWritable({keepExistingData:true})` trên một file mà writable phiên TRƯỚC (không phải phiên `commit()` của chính job này) CHƯA từng `close()` (đúng kịch bản crash/reload giữa chừng) — chưa rõ có đụng cùng khoá cấp thấp từng gây `InvalidStateError` ở §8.9/§8.10 hay không.
