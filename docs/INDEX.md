...
25 source directories · 145 source files · 37 co-located test files.
...
|---|---|---|---|---|---|
| `src/adapters/browser-extension/background` | 1 | — | — | Composition root for the service worker — constructs the Kernel and wires every background listener | `kernel-bootstrap` |
...
| `src/adapters/browser-extension/features/media` | 12 | — | MAIN world, background, content | Media detection (webRequest, DOM, MSE/HLS hooks) and the network sniffer Module | `features/media/.domain.md` |
| `src/adapters/browser-extension/features/media/download` | 12 | — | background, offscreen | Download engine: VOD/live/turbo jobs, segment pipeline, OPFS staging, ffmpeg remux | `features/media/.domain.md` |
...
| `src/shared` | 19 | 10 | — | Global SDK — pure functions, zero side effects; the only layer a MAIN-world payload may import | `sdk-layers` |
