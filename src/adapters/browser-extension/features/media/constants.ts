 ...
 * in their own file so content-scripts/index.ts and dom-media-observer.content.ts (the ISOLATED-world
 ...
 * media-source-interceptor.page.ts's hook, ISOLATED world (dom-media-observer.content.ts) uses them to anchor a
...
/** docs/ROADMAP.md #7.3(a-hls) — set directly on a `<video>` element by hls-global-hook.page.ts's
 ...
 * worlds, so the attribute value is readable by dom-media-observer.content.ts as soon as it's set — but
 * being READABLE doesn't mean a rescan gets TRIGGERED: dom-media-observer.content.ts's MutationObserver is
 ...
 * this is needed: the attribute write alone doesn't trigger dom-media-observer.content.ts's rescan. Empty
