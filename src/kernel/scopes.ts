...
export const API_METHODS: ApiMethodDefinition[] = [
  ...
  // docs/ROADMAP.md Track A2 — `storage.tab.*`/`storage.session.*`, same dotted-method dispatch
  // `net.mock.add` already established (`rpc-handler.ts`'s `resolveMethodHandler` walks a dotted
  // method name down from the namespace object) — same scope (`storage.rw`, this is a lifetime
  // axis, not a permission axis), same 4 operations, different eviction trigger.
  {
    namespace: 'storage',
    method: 'tab.get',
    scope: 'storage.rw',
    signature: 'get(key: string): Promise<unknown>',
    description: "Read one of this script's own tab-scoped keys. Resolves to `undefined` when unset.",
    transport: 'rpc',
  },
  {
    namespace: 'storage',
    ...
    scope: 'storage.rw',
    signature: 'set(key: string, value: unknown): Promise<void>',
    description: 'Write one tab-scoped key — evicted when the calling tab closes, survives navigation within it.',
    transport: 'rpc',
  },
  {
    namespace: 'storage',
    ...
    scope: 'storage.rw',
    signature: 'remove(key: string): Promise<void>',
    description: 'Delete one tab-scoped key.',
    transport: 'rpc',
  },
  {
    namespace: 'storage',
    ...
    scope: 'storage.rw',
    signature: 'keys(): Promise<string[]>',
    description: "List this script's own tab-scoped keys for the calling tab.",
    transport: 'rpc',
  },
  {
    namespace: 'storage',
    ...
    scope: 'storage.rw',
    signature: 'get(key: string): Promise<unknown>',
    description: "Read one of this script's own session-scoped keys. Resolves to `undefined` when unset.",
    transport: 'rpc',
  },
  {
    namespace: 'storage',
    ...
    scope: 'storage.rw',
    signature: 'set(key: string, value: unknown): Promise<void>',
    description: 'Write one session-scoped key — evicted on the calling tab\'s next navigation commit, including a same-URL reload.',
    transport: 'rpc',
  },
  {
    namespace: 'storage',
    ...
    scope: 'storage.rw',
    signature: 'remove(key: string): Promise<void>',
    description: 'Delete one session-scoped key.',
    transport: 'rpc',
  },
  {
    namespace: 'storage',
    ...
    scope: 'storage.rw',
    signature: 'keys(): Promise<string[]>',
    description: "List this script's own session-scoped keys for the calling tab.",
    transport: 'rpc',
  },
  {
    namespace: 'ui',
